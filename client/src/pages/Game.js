...
const getEffectDescription = (card, type) => {
  ...
  switch (effectType) {
    ...
    case 'level2_any':    return { tag: '⬆️ Level 2 Upgrade', desc: 'Added to your hand. Use to instantly upgrade any Level 1 property to Level 2!' };
    case 'color_l1':      return { tag: `🎨 ${color?.toUpperCase()} Level 1`, desc: `Added to your hand. Instantly upgrade any base-level ${color} property to Level 1!` };
    default:              return { tag: '🃏 Card Drawn',       desc: card.text || 'Effect applied.' };
  }
};
 
// Whether the card goes to the player's hand vs. instant effect
...
const EventCardModal = ({ cardEvent, me, onDismiss }) => {
    ...
    <div style={EC.overlay}>
      <div style={{ ...EC.modal, borderColor: theme.border }}>
        ...
        <div style={EC.body}>
          ...
          {/* Card text (the flavour/rule text) */}
          {card?.text && !noSupply && (
          ...
          {/* Updated balance — only for money events */}
          ...
          <button onClick={onDismiss} style={{ ...EC.btn, background: theme.accentDim, border: `1px solid ${theme.border}`, color: theme.accent }}>
            ✓ Got it
          </button>
        </div>
      </div>
...
const EC = {
  ...
  modal:       { background:'#0f2035', border:'1px solid', borderRadius:16, width:330, maxWidth:'95vw', overflow:'hidden', boxShadow:'0 24px 64px rgba(0,0,0,0.7)' },
...
const getInitialState = () => {
  ...
  const [cardEvent,     setCardEvent]     = useState(null); // ← NEW for EventCardModal
  ...
  if (gs) sessionStorage.setItem('monopoly_gamestate', JSON.stringify(gs));  // ← add `if (gs)`
  setGameState(gs);
  setStatus('playing');
};
  ...
  if (gs) sessionStorage.setItem('monopoly_gamestate', JSON.stringify(gs));
      ...
      if (cardTypes.includes(gs.lastEvent?.type) && gs.lastEvent?.playerId === me?.id) {
        setCardEvent({
          ...
          cardId:    gs.lastEvent.cardId || null,   // for upgrader_no_supply
...
{(() => {
  ...
  return (
    <div style={JM.overlay}>
      ...
      {showPropPopup && isMyTurn && currentPhase === 'action' && (
        <PropertyPopup
          gameState={gameState}
          me={me}
      ...
      <div style={S.panel}>
        ...
        {isMyTurn && (
          <div style={S.actions}>
            ...
            {currentPhase === 'action' && !showPropPopup && <>
