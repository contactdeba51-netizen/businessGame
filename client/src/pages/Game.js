...
// ── DiscountPayModal ───────────────────────────────────────────
const DiscountPayModal = ({ event, me, onUseCard, onPayFull, onDismiss }) => {
  if (!event) return null;
  return (
    <div style={PP.overlay}>
      <div style={{ background: '#0f2035', border: '1px solid rgba(16,185,129,0.4)', borderRadius: 16, width: 320, maxWidth: '95vw', overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}>
        <div style={{ padding: '20px', background: 'rgba(16,185,129,0.08)', borderBottom: '1px solid rgba(16,185,129,0.2)', textAlign: 'center' }}>
          <div style={{ fontSize: 32, marginBottom: 4 }}>🎟️</div>
          <div style={{ color: '#6ee7b7', fontWeight: 800, fontSize: 17 }}>Discount Pay Card</div>
          <div style={{ color: '#8fb8d8', fontSize: 11, marginTop: 4 }}>You owe rent to {event.ownerName}</div>
        </div>
        <div style={{ padding: '16px 20px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '10px 12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: '#8fb8d8', fontSize: 11 }}>Full rent</span>
              <span style={{ color: '#fca5a5', fontWeight: 700 }}>₹{event.rentDue?.toLocaleString()}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
              <span style={{ color: '#8fb8d8', fontSize: 11 }}>With 50% discount</span>
              <span style={{ color: '#6ee7b7', fontWeight: 800 }}>₹{event.discountedRent?.toLocaleString()}</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <button onClick={onUseCard} style={{ flex: 2, background: '#065f46', color: '#fff', border: 'none', borderRadius: 10, padding: '11px', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
              🎟️ Use Card (₹{event.discountedRent?.toLocaleString()})
            </button>
            <button onClick={onPayFull} style={{ flex: 1, background: 'rgba(255,255,255,0.08)', color: '#fff', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 10, padding: '11px', fontWeight: 600, fontSize: 11, cursor: 'pointer' }}>
              Pay ₹{event.rentDue?.toLocaleString()}
            </button>
          </div>
        </div>
      </div>
...
const Game = () => {
  ...
  const [showDiscountBuyPrompt, setShowDiscountBuyPrompt] = useState(false);
  ...
  useEffect(() => {
    ...
    const onStateUpdated = ({ gameState: gs }) => {
      ...
      // ADD THIS BLOCK — after the cardTypes block, before the action phase block
      if (gs.lastEvent?.type === 'can_use_discount_pay' && gs.lastEvent?.payerId === me?.id) {
        setDiscountPayEvent({
          rentDue: gs.lastEvent.rentDue,
          discountedRent: gs.lastEvent.discountedRent,
          ownerName: gs.players?.find(p => p.id === gs.lastEvent.ownerId)?.username || '?',
          propertyName: gs.lastEvent.propertyName || currentSquare?.name || '',
        });
        setShowPropPopup(false);
        return;
      }
  ...
  const handleBuy = () => {
    const hasDiscountBuy = me?.heldCards?.some(c => c.cardId === 'discount_buy');
    if (hasDiscountBuy) {
      setShowDiscountBuyPrompt(true); // show a small confirm dialog
    } else {
      emit('buy_property');
      setShowPropPopup(false);
    }
  ...
  return (
    <div style={S.root}>
      ...
      {/* ── Discount Pay Modal ── */}
      {discountPayEvent && isMyTurn && (
        <DiscountPayModal
          event={discountPayEvent}
          me={me}
          onUseCard={() => { emit('use_discount_pay'); setDiscountPayEvent(null); }}
          onPayFull={() => {
            // Manually trigger a processRentPayment by sending a synthetic skip
            // The server already has the pending rent — we just need a "pay full" signal.
            // Simplest: emit a custom action or re-use skip_buying (doesn't fit).
            // BEST: add 'pay_rent_full' action to socket.js (see note below).
            emit('pay_rent_full');
            setDiscountPayEvent(null);
          }}
          onDismiss={() => setDiscountPayEvent(null)}
        />
      )}
      ...
      {/* ── Discount Buy Prompt ── */}
      {showDiscountBuyPrompt && currentSquare && (
        <div style={PP.overlay}>
          <div style={{ background: '#0f2035', border: '1px solid rgba(212,160,23,0.4)', borderRadius: 14, width: 300, maxWidth: '95vw', padding: 20, textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.7)' }}>
            <div style={{ fontSize: 28, marginBottom: 8 }}>🏷️</div>
            <div style={{ color: '#fde68a', fontWeight: 800, fontSize: 16 }}>Use Discount Buy Card?</div>
            <div style={{ color: '#8fb8d8', fontSize: 11, marginTop: 6, marginBottom: 14 }}>
              Buy {currentSquare.name} for{' '}
              <span style={{ color: '#fca5a5', textDecoration: 'line-through' }}>₹{currentSquare.buyPrice?.toLocaleString()}</span>
              {' → '}
              <span style={{ color: '#6ee7b7', fontWeight: 800 }}>₹{Math.floor(currentSquare.buyPrice / 2)?.toLocaleString()}</span>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => { emit('buy_property', { useDiscountCard: true }); setShowDiscountBuyPrompt(false); setShowPropPopup(false); }}
                style={{ flex: 1, background: '#065f46', color: '#fff', border: 'none', borderRadius: 9, padding: '10px', fontWeight: 700, cursor: 'pointer' }}>
                🏷️ Use Card
              </button>
              <button onClick={() => { emit('buy_property', { useDiscountCard: false }); setShowDiscountBuyPrompt(false); setShowPropPopup(false); }}
                style={{ flex: 1, background: 'rgba(255,255,255,0.08)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 9, padding: '10px', fontWeight: 600, cursor: 'pointer' }}>
                Pay Full
              </button>
            </div>
          </div>
